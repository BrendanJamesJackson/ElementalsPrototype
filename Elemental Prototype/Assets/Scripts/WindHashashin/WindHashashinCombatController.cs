...
public class WindHashashinCombatController : MonoBehaviour
{
    ...
    [SerializeField]private bool SecondaryActivated = false;
    public bool HoldAttackActivated = false;

    public BoxCollider2D swordsCollider;
    ...
    public void SecondaryAttackInputHandler(InputAction.CallbackContext context)
    {
        if (context.performed)
        {
            ...
            else if (SecondaryActivated /*&& timeSinceSecondary > 0.05f*/)
            {
                ThirdAttack();
                Debug.Log("Secondary");
                SecondaryActivated = false;
